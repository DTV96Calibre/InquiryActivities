...
var waterZoomPlaying = false;
var dyeZoomPlaying = false;
...
function init() {
	...
	$("#waterZoomLink").live('click', showWaterZoom);
	$("#dyeZoomLink").live('click', showDyeZoom);
}
...
function generateWaterZoom() {
	var zoomHTML = '<div id="waterZoomDiv">';
...
function showWaterZoom() {
	
	if(waterZoomPlaying) {
		$("#zoomInBackdrop").hide();
		$("#waterZoomDiv").hide();
		waterZoomPlaying = false;
	}
	else {
		waterZoomPlaying = true;
		$("#cold1").css("top", "1px");
		$("#cold1").css("left", "-9px");
		$("#hot1").css("top", "1px");
		$("#hot1").css("left", "11px");
...
function randomizeColdPosition(name) {
	...
	while( (Math.pow(x, 2)+Math.pow(y, 2)) > 9700) {
		x = -Math.random()*100;
		y = Math.random()*100;
		if (Math.random()>0.5) {
			y = y*-1;
...
function animateWaterZoom() {
	...
	for(var i=1; i<=250; i++) {
		...
		if(i==250) {
			...
			$(name).animate({left:xmove, top:ymove}, 5, "linear", animateWaterZoom);
		}
		else {		
			$(name).css({left:xmove, top:ymove});
		}
...
function determineXMove(x, y) {
	...
	if (Math.pow((x), 2)+Math.pow((y), 2)>9700) {
		if(x > 0)
			xmove = -3;
		else
			xmove = 3;
...
function showDyeZoom() {
	
	if(dyeZoomPlaying) {
		$("#zoomInBackdrop").hide();
		$("#dyeZoomDiv").remove();
		dyeZoomPlaying = false;
	}
	else {
		dyeZoomPlaying = true;
